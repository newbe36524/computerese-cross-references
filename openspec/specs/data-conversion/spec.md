...
## Purpose
TBD - created by archiving change data-yaml-multi-format-converter. Update Purpose after archive.
## Requirements
...
- **GIVEN** the conversion CLI command `convert` exists
...
- **GIVEN** the conversion CLI command `convert` exists
- **WHEN** executed with `--format markdown --format html --format pdf` (multiple format arguments)
...
Markdown and HTML formats SHALL use Handlebars templates for customizable styling and structure.
